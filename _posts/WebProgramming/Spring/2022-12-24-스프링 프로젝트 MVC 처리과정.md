...
5. 로딩된 root-context에 등록된 Spring 컨테이너가 구동되며 Service, dao 등의 객체도 함께 생성된다.

### (2) 클라이언트로부터 요청이 왔을 때
6. DispatcherServlet이 구동된다. 이때 Servlet으로 선언되었기 때문에 해당 Servlet 내에서만 사용가능한 Context가 된다.
7. DispatcherServlet이 servlet-context.xml을 로딩한다.
8. 로딩된 servlet-context에 등록된 Spring 컨테이너(Spring Container) 가 구동되며 등록된 Controller 객체를 생성한다.
