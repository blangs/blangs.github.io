...
- @Controller  
  : 해당 클래스가 Controller라고 지정한다.  
- @RequestMapping  
  : 웹 브라우저의 요청 url을 메소드 단위로 처리하여 해결한다.

> **패키지 생성방법)**  
> src 우클릭 - spring.home.controller 입력 - 패키지생성
>  
> 컨트롤러 생성까지 완성하면 아까전에 servlet-context.xml 에서 등록한 컨트롤러 bean 에서 나던 오류가 사라진다.
