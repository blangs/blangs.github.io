...
3. Libraries 탭 - Add JARs..
  - 아까 넣은 라이브러리리들을 모두 추가한다.
  - 폐쇠망등을 고려하여 어디서든 패키징하여 사용할 수 있도록 상대경로로 추가한다.
...
> 예를 들면 톰캣의 Server.xml 파일에서 Context Path를 dymmm 이라고 부여했다고 가정한다.  
> www.test.com/dymmm 이 기준이되어 작동할것이다.  
> 이 기준점에서 위에서 설명한것처럼 파일에 접근할때 Dispatcher Servlet 매핑을 거치도록 한번 더 고려하면 된다. 
