...
//String strJsonStr = "[제이슨 형태 데이터]";
ArrayList<Map<String, Object>> listCluMt = new ArrayList<Map<String, Object>>();
Map<String, Object> mapJson = gson.fromJson("[제이슨 형태 데이터]", mapType);
Map<String, Object> mapData = (Map<String, Object>) mapJson.get("[루트key ACCESS]");  //ex) "clusterMetrics"

if (mapData != null) {  
  for (String keyName : mapData.keySet()) {
    dataEntry.put(keyName, mapData.get(keyName));
    
    // ArrayList<Map<String, Object>> 데이터를 생성했다!
    listCluMt.add(dataEntry); 
  }
}

// JSP (프론트로) 전달
model.addAttribute("listCluMt", listCluMt.toString());
...
$(document).ready(function (){
	...
	// 온로드시 백앤드에서 전달받은 JSON 처리
	listCluMt = '${listCluMt}'; // Original JSON String
	var cluMtData = JSON.parse(listCluMt);
	
	var stastus = cluMtData.documnet[0].status;
	var msg = cluMtData.documnet[0].msg;
	var address = cluMtData.documnet[0].address[0].data1;
	var language = cluMtData.documnet[0].language[0].언어;
});
</script>
