...
2. dotenv 라이브러리를 설치
3. .env 파일을 로드합니다.
    - require('dotenv').config()와 같이 사용합니다.
...
## STEP3. config 라이브러리 사용하기
config 라이브러리를 설치합니다. npm install config와 같이 사용합니다.
프로젝트 루트 디렉토리에 config 폴더를 만듭니다.
config 폴더 안에 default.json 파일을 만듭니다.
default.json 파일에 필요한 설정 값을 정의합니다. 예를 들어, "DATABASE_URL": "postgres://user:password@localhost:5432/mydb"와 같이 변수를 설정합니다.
config 라이브러리를 사용하여 설정 값을 가져옵니다. const config = require('config'); const dbUrl = config.get('DATABASE_URL');와 같이 사용합니다.
이 외에도 rc, nconf 등 다양한 라이브러리를 사용하여 환경 변수를 설정할 수 있습니다.
