...
만약 create-react-app으로 프로젝트를 생성했다면, src/index.js 파일이 엔트리포인트이다. 이 파일에서 ReactDOM.render() 메서드를 호출하여 리액트 컴포넌트를 렌더링합니다. 따라서 index.js 파일에서 ReactDOM.render() 메서드를 찾아보면 엔트리포인트를 확인할 수 있다.
...
>   target: 'http://localhost:8080', // 스프링부트 서버 URL
>   changeOrigin: true,
> });
> 
