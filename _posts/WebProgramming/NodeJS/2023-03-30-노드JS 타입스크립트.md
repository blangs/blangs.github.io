...
// TypeScript 프로젝트를 컴파일할 때, 해당 설정에 따라 TypeScript 컴파일러가 동작한다. 
{
  "compilerOptions": {
    ...
    "module": "commonjs",  
                             ...
                             // ex) import 구문으로 CommonJS 모듈을 가져올 수 있고, 
                             //     export default 구문으로 ES 모듈을 내보낼 수 있게된다.
    "outDir": "./dist",      // 컴파일된 JavaScript 파일의 출력 디렉토리를 지정.
    "sourceMap": true        // 소스 맵을 생성하여 디버깅을 용이하게 합니다.
  },
  "include": [
    "./src/**/*.ts"
  ],
...
- `"module": "commonjs"`  
    :  모듈 시스템을 지정. "commonjs"는 Node.js 환경에서 일반적으로 사용되는 CommonJS 스타일의 모듈 시스템을 지정한다. commonjs 인 경우는 require와 module.exports를 사용하여 모듈을 정의하고 내보내는 방식으로 동작을 의미한다.

    ```js
    // "commonjs" 으로 모듈 시스템을 지정
    // 컴파일 명령을 실행하면 example.ts 파일이 example.js 파일로 컴파일되며, 해당 파일은 Node.js에서 require 함수를 사용하여 로드하고 내보낸 함수를 사용할 수 있게 된다.
...
- "esModuleInterop"  
    : true, ES 모듈과 CommonJS 모듈 간의 상호 운용성을 활성화 개선. (대부분의 경우 true)

    ```bash 
    # "esModuleInterop" 옵션 true 인 경우
    TypeScript 컴파일러가 ES 모듈과 CommonJS 모듈 간의 상호 운용성을 개선 import 구문으로 CommonJS 모듈을 가져올 수 있고, export default 구문으로 ES 모듈을 내보낼 수 있
