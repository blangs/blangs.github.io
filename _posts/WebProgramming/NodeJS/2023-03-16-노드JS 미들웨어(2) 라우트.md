...
## 라우터 컨트롤러 구조잡기
***참고) [바로가기](https://firework-ham.tistory.com/59)***  
{: .notice--info}
...
이렇게 변경하면 url 변경 또는 router 파일 변경시 <span style='color:red'>app.js를 수정하지 않고 /router/index.js를 수정하면 url 및 api 관리가 가능해진다.</span> 정말 간단하게 변경했지만 관리하는 관점에서는 대폭 개선된다. 정상적으로 api가 호출되는지 한번 확인한다.
