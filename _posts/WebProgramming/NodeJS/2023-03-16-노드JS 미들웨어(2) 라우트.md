...
`http://호스트주소:포트/main/login` 요청시 정상 작동한다.  
...
결과는 정상 작동한다.  
<span style='color:red'>객체를 등록하고 내용을 작성해도 무방하다.</span>
...
// 2. 라우터 객체내용 작성
router.get('/', (req, res) => {
    console.log('/ 주소의 [GET] 요청일 때 실행된다.');
    res.send("고객정보조회");
})
.post('/insert', (req, res) => {
    console.log('/insert 주소의 [POST] 요청일 때 실행된다.');
    res.send("신규고객추가");
})
.put('/update', (req, res) => {
    console.log('/update 주소의 [PUT] 요청일 때 실행된다.');
    res.send("고객정보수정");
})
.delete('/delete', (req, res) => {
	console.log('/delete 주소의 [DELETE] 요청일 때 실행된다.');
    res.send("기존고객삭제");
});
...
메인 어플레케이션이 매우 간소화 되었다. 길이가 길 수록 더욱 코드는 간결해진다.
...
|app.get('/path, 미들웨어)|path로 시작하는 GET 요청에서 미들웨어 실행|
