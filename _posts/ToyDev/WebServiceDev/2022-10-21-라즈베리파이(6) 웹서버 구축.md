...
4. 소스파일 압축해제
  : 각각의 폴더에 압축을 풀어준다. 
    ...
    /fswas/apache/httpd# tar -xvfz httpd-2.4.51.tar.gz
...
5. 설치
  : 이제 설치를 위한 준비가 끝났다. 설치를 진행한다.
    ...
    : 아파치를 의존성패키지와 함께 설치한다.
      ...
      ```bash
      cd /fswas/apache/httpd-2.4.541
  
      ./configure --prefix=/fswas/apache/httpd \  #Apache를 설치할 경로를 입력한다. --prefix= 에 입력하는 경로에 Apache가 설치된다.
      --enable-module=so \
      --enable-rewrite \
