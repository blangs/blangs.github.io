...
### 테스트
  ...
  2. 테스트페이지에서 최초 세션맺기 ==> WAS1 또는 WAS2 로 들어가서 세션을 맺어준다
  
  3. 새로고침 해본다 ==> 똑같은 (ex.WAS1) 서버로 요청할것이다.. sticky 밸런싱 정상작동 확인

  4. was1 서버를 강제로 중지시킨다. ==> 세션클러스터링 확인을 위해

  5. was2 서버의 catalina.out 로그를 확인해본다. ==> was1의 세션을 복사하는것이 보인다.

  6. 다시 테스트페이지로 접속해보면 서버는 달라졌지만 동일한 세션ID가 찍히는것을 볼 수 있다.
