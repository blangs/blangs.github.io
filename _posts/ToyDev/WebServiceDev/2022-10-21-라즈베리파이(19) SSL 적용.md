...
1. 원인(2)
  : JkMount(mod_jk.so)를 이용해서 Tomcat과 연동되는 경우, 80포트의 JkMount 설정을 SSL 관련 VirtualHost에 동일하게 복사해야 한다고 한다. 즉, 기존에 80포트에 설정(httpd.conf)해둔 jkMount 내용을 ssl설정파일(httpd-ssl.conf) 안에 동일하게 작성해주어야 한다

2. 해결방안(2)
  : (아파치루트/conf/httpd.conf) 안에 기존에 80포트에 설정한 JkMount 를 그대로 복사해서 붙여넣는다.  
