...
1. 원인  
  : 현재 발급받은 SSL인증서가 www.blang.co.kr 1개 이다. 그래서 나의 경우는 blang.co.kr 으로는 적용되지 않는 환경이다. 즉, SSL발급시점에 메인도메인,서브도메인중에 서브도메인 하나만 발급한 경우이다.

    > `메인도메인` 과 `www.서브도메인` 은 같은 도메인으로 치는 경향이 있으니 -d blang.co.kr -d www.blang.co.kr 처럼 둘 모두 작성해야한다.
