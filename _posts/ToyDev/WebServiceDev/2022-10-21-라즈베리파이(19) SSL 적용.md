...
1. 원인  
  : 현재 발급받은 SSL인증서가 www.blang.co.kr 1개 이다. 그래서 나의 경우는 blang.co.kr 으로는 적용되지 않는 환경이다. 이 부분은 좀더 상세하게 설명한다.

- SSL발급시점에 메인도메인,서브도메인 하나만 발급한 경우   
  : `메인도메인` 과 `www.서브도메인` 은 같은 도메인으로 치는 경향이 있으니 -d swiftcoding.org -d www.swiftcoding.org 처럼 둘 모두 작성해야한다.
    ...
    # 아래처럼 했어야 했었다.
    # 메인도메인과 서브도메인을 모두 발급받아야 했었다.
    sudo certbot certonly --manual --email layup3@naver.com -d www.blang.co.kr -d blang.co.kr

    ```
    
    참고) 현재 DNS 간단하게 상황보기  
    DNS가 잘작동하는지 간단하게 주소만입력하면 확인할 수 있다.
    https://toolbox.googleapps.com/apps/dig/#A/
    {: .notice--info}

2. 해결방안  
  : 리다이렉트 모듈을 추가하고 기능을 적용한다. 이미 SSL 1개만 받았으므로 어쩔수 없다. 어떤 주소를쳐도 SSL이 적용된 `www.도메인` 형태로 요청하도록 만들면 될것이다.  
    ...
    <VirtualHost *:80>
        ...
        RewriteRule (.*) https://www.blang.co.kr%{REQUEST_URI}
