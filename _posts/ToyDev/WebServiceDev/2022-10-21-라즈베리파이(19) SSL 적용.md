...
1. 원인(2)
  : 작성한 SSL설정파일인 httpd-ssl.conf 안에 ServerName 이라는 요청으로 들어오는 특정도메인에 대한 설정부분이있다. 나는 이전에 www.도메인.co.kr 으로 작성했었다. 이 부분도 명확하게 분리시켜야 하나보다. 
  ...
  : (아파치루트/conf/extra/httpd-ssl.conf) 안에서 www를 사용하지 않는 요청에대한 `<VirtualHost>` 태그를 새롭게 하나더 만들어준다.
    ...
    ```bash     
      <VirtualHost _default_:443>
          ...
          DocumentRoot "/fswas/apache/httpd/httpd-2.4.541/htdocs"
          ServerName blang.co.kr:443    # www 를 제거한다.
