...
1. 원인  
  : JkMount(mod_jk.so)를 이용해서 Tomcat과 연동되는 경우, 80포트의 JkMount 설정을 SSL 관련 VirtualHost에 동일하게 복사해야 한다고 한다. 즉, 기존에 80포트에 설정(httpd.conf)해둔 jkMount 내용을 SSL(443) 포트에도 jkMount 동일하게 작성(httpd-ssl.conf) 해주어야 한다. 

2. 해결방안  
  : (아파치루트/conf/extra/httpd-ssl.conf) 안에 80 포트에만 설정해두었던 `JkMount` 를 그대로 복사해서 붙여넣는다.  

    ```bash  
    # httpd.conf 안에서 설정한 JkMount 내용을 복사한뒤 ssl설정파일 OPEN
...
- 두번째시점) 입력한 url을 https으로 rewrite 처리를 안함
    ...
    > 방법도 여러가지인데 대중적인 방법은 `mod_rewrite.so` 모듈을 사용한 리다이렉트를 구현한다.
...
2. 해결방안  
  : 리다이렉트 모듈을 추가하고 기능을 적용한다. 어떤 주소를쳐도 SSL이 적용된 `www.도메인` 형태로 요청하도록 처리한다.
