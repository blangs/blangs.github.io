...
1. 파일 자체의 인코딩을 변경
    ...
    - 문서 내부의 한글이 모두 깨졌다. 
      ...
      ```bash
      iconv -f utf-8 -t euc-kr index_html.html > converted.html
      file -bi converted.html
...
2. 파일 자체의 인코딩을 무시하고 한글로 저장
    - 한글로 바꾸고 저장
    - 우리 눈에는 에디터로 즉시 수정하고 저장누르기 때문에 한글로 저장되는 것처럼 보인다.\
    - 참고로 vi에디터는 이런경우를 방지하게끔 저장못하게 알려준다. 
    - 처음에는 이 부분이 정말 멘붕이 오는 구간인듯.

3. 테스트
    - 파일을 닫고 다시 오픈한다: 당연히 파일인코딩이 한글을 지원하지 않으니까 변경된다.
    - 
