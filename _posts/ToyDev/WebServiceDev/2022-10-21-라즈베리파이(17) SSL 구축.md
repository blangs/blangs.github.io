...
> https는 제 3자인 CA(인증기관)가 보증하는 문서를 통해 보안 통신을 제공한다. 이 문서를 SSL 인증서라 한다.
> 정리하면 https는 SSL 프로토콜 위에서 돌아가는 프로토콜이며 SSL은 데이터를 안전하게 전송하기 위한 인터넷 통신 규약 프로토콜이다.
> SSL은 2.0과 3.0 버전을 거쳐 현재는 SSL 3.0을 기반으로한 TLS가 도입되어 사용 중이다.
