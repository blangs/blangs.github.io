...
### 인증서 생성
  ...
  - 참고) 아파치 수동 컴파일 환경에서 실패한 내용
    : 특정경로에 아파치를 소스컴파일 방식으로 설치한 환경에서 일반적인 인증서 발급방법이 모두 실패했다. (오랜 시간동안 삽질..) 해당 내용은 다음과 같다.
...
___
    ...
    > ***삽질)***  
    > 나의 경우에는 3번째 방법으로 일부 진행이 되기는했다.
    > 삽질끝에 결론적으로는.. 도메인 소유권인증이 잘되지 않았다.


  1. 도메인소유권 인증을 하드코딩으로 인증서발급받기
    : 지금까지 에러가난 이유는 소유권 인증을 못해서 그렇다는것같다.
      
      - ✨첫번째 터미널 (Ubuntu 서버 접속후) 키값 얻기 
      - 
...
### 웹서버 로드밸런싱 (JkMount) 매핑 설정
  : 우선 SSL 적용하는 곳은 웹서버이다. 그런데 현재 모든 요청이 WAS 측으로 넘어가도록 로드밸런싱 되어있다. 특정 요청은 넘어가지 않도록 JkMount 매핑을 설정해주도록 해야한다.
