...
// 적용방법은 다음과 같이 업무에 맞는 형태로 JAVA에서 처리한다.
select CONCAT('M', lpad(select nextval(SQ_TBDBDW001_01),7,0)) slfMmsn;

```
select CONCAT('M', lpad(select lastval(SQ_TBDBDW001_01),7,0)) slfMmsn;
...
// 테이블 데이터 삽입
// ------------------------------
MariaDB [(INSTC)]> INSERT INTO test VALUE(
    -> ' 1'
    -> , '최태호'
    -> , '1993-04-23'
    -> , 0
...
MariaDB [(INSTC)]> select * from INSTC.test
...
Query OK, 1 row affected (0.034 sec);
+------+-----------+--------------+-----------+
| id   | name      | sysLstUpdYms | pwdErrCnt |
+------+-----------+--------------+-----------+
|    1 | 최태호    | 1993-04-23   |         0 |
+------+-----------+--------------+-----------+
1 row in set (0.001 sec)
