---
title:  "라즈베리파이(14) WEB WAS 로드밸런싱"
excerpt: "라즈베리파이(14) WEB WAS 로드밸런싱 입니다."
categories:
  - web-service-dev
tags:
  - [web, dev]
...
### 연동완료 정리
  : mod_jk 를 이용한 로드밸런싱이 완료되었다. was1번,2번에 동일한 url서블릿 매핑을 가진 jsp주소를 호출해보면 (내용은 달라야한다) 서로 다른내용이 번갈아가며 출력되는것을 볼 수 있다.
  
![사진2](/assets/images/ToyDev/WebServiceDev/mod_jk_success_loadbalancer.png)

> ***참고) 클러스터링이 필요하다.***  
> 서버가 번갈아가면서 처리되는걸 보면 목적대로 로드밸런싱은 성공했지만
> 세션이 공유되어야하는 경우 클러스터링 작업이 별도로 필요하다.
> 클러스터링이란 한 세션이 맺어지면 끊어지지않는이상 계속 동일한 서버를 바라보도록 처리한다.
