...
- 충돌이 발생한 파일은 Git에 의해 변경사항이 표시되어 있다.
...
```bash
git add public/study-file1.txt
git commit
```

<span style="color:green"><I><b>머지(병합)</b></I></span>  
> 커밋을 완료하면 머지가 성공적으로 완료됩니다. 추가로 해결해야 할 충돌이 있다면 위의 단계를 반복하면 됩니다.
